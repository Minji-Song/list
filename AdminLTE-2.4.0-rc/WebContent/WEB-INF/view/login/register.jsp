...
<form action="join.do" method="post">
      ...
      <div class="form-group has-feedback">
        <input type="email" class="form-control" placeholder="Email" name="id" value="${param.id}">
        <c:if test="${errors.id}">이메일을 입력하세요.</c:if>
		<c:if test="${errors.duplicateId}"><script>alert("이미 사용중인 이메일입니다.");</script>	</c:if>
      ...
      <div class="form-group has-feedback">
        ...
        <c:if test="${errors.password}">비밀번호 확인을 입력하세요.</c:if>
        <c:if test="${errors.notMatch}"><script>alert("비밀번호와 확인이 일치하지 않습니다.");</script></c:if>
